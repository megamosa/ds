...
define([
    ...
    return {
        ...
        /**
         * Enhanced shipping methods rendering with proper price handling
         */
        renderShippingMethods: function(methods) {
            ...
            if (!methods || methods.length === 0) {
                container.html(
                    ...
                    '<span>لا توجد طرق شحن متاحة لهذا الموقع. يرجى التحقق من بيانات العنوان.</span>' +
                    '</div>'
                );
                return;
            }
            
            let html = '<div class="shipping-methods-list">';
            
            methods.forEach(function(method, index) {
                const isChecked = index === 0 ? 'checked="checked"' : '';
                const priceDisplay = method.price > 0 ? method.price_formatted : 'مجاني';
                ...
                html += `
                    <div class="shipping-method-option">
                        <input type="radio" 
                               ...
                               data-price-formatted="${method.price_formatted}"
                               ${isChecked} />
                        <div class="method-info">
                            <div class="method-title">${method.carrier_title} - ${method.title}</div>
                            ${method.description ? `<div class="method-description">${method.description}</div>` : ''}
                        </div>
                        <div class="method-price">${priceDisplay}</div>
                    </div>
                `;
            });
            ...
            // Auto-select first method and update calculation
            if (methods.length > 0) {
                this.updateShippingCostDisplay();
                this.updateCalculation();
            }
        },
        
        /**
         * Update shipping cost display immediately when method changes
         */
        updateShippingCostDisplay: function() {
            ...
            if (selectedMethod.length > 0) {
                ...
                // Update shipping cost in order summary immediately
                $('#magoarab-shipping-cost').text(priceFormatted);
                
                // Update total immediately (basic calculation)
                this.updateOrderTotalDisplay();
            }
        },
        
        /**
         * Update order total display with current values
         */
        updateOrderTotalDisplay: function() {
            const subtotalElement = $('#magoarab-subtotal');
            const shippingElement = $('#magoarab-shipping-cost');
            const totalElement = $('#magoarab-order-total');
            
            if (subtotalElement.length && totalElement.length) {
                const subtotalText = subtotalElement.text().replace(/[^\d.]/g, '');
                const shippingPrice = parseFloat($('input[name="shipping_method"]:checked').data('price')) || 0;
        ...
        formatPrice: function(price) {
            // Use Magento's price formatting if available
            if (typeof window.checkoutConfig !== 'undefined' && 
                window.checkoutConfig.priceFormat) {
                return window.checkoutConfig.priceFormat.pattern.replace('%s', price.toFixed(2));
            }
            
            // Fallback formatting
            return price.toFixed(2) + ' ' + (this.config.currencySymbol || 'ر.س');
        },
...
bindEvents: function() {
    ...
    // Enhanced shipping method change
    $(document).on('change', 'input[name="shipping_method"]', function() {
        console.log('Shipping method changed:', $(this).val());
        
        // Immediate UI update
        self.updateShippingCostDisplay();
        
        // Full calculation with rules
        self.updateCalculation();
