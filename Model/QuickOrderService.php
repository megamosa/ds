...
public function createQuickOrder(QuickOrderDataInterface $orderData): array
{
    ...
    try {
        ...
        // Get frontend values to force exact match
        $frontendValues = $orderData->getData('frontend_values');
        
        $this->logger->info('=== STEP 8: Processing frontend values ===', [
            'frontend_values_received' => $frontendValues,
            'has_frontend_values' => !empty($frontendValues),
            'is_array' => is_array($frontendValues)
        ]);
        
        if ($frontendValues && is_array($frontendValues)) {
            $this->logger->info('Using frontend values for exact match', $frontendValues);
            $this->forceFrontendTotalsOnOrder($quote, $frontendValues);
        } else {
            $this->logger->info('No frontend values - applying rules normally');
            // Fallback: Ensure all rules are properly applied
        ...
        // STEP 10.2: Add order note if provided
        $orderNote = $orderData->getOrderNote();
        if ($orderNote) {
            $order->addStatusHistoryComment(
                __('Customer Note: %1', $orderNote),
                false, // Don't notify customer  
                true   // Set as visible on frontend
            );
        ...
        // If frontend values exist, only adjust totals while preserving rule information
        if ($frontendValues && is_array($frontendValues)) {
            $this->logger->info('=== STEP 10.3: Adjusting totals while preserving rules ===');
            $this->adjustOrderTotalsPreservingRules($order, $frontendValues);
        }
...
private function setAddressData($address, QuickOrderDataInterface $orderData, string $customerEmail): void
{
    ...
    public function calculateShippingCost(
        ...
        try {
            ...
            // CRITICAL FIX: Don't use default shipping price for dynamic calculation
            // This was causing the static 50 value. We should return 0 and let the
            // dynamic calculation handle the correct shipping cost
            $this->logger->error('CRITICAL: calculateShippingCost fallback - returning 0 for dynamic calculation', [
...
public function calculateOrderTotalWithDynamicRules(
    ...
    try {
        ...
        // Apply catalog rules first (affects product prices)
        $this->applyCatalogRules($quote);
        
        // Then apply cart rules (affects totals)
        $this->applyCartRules($quote);
        
        // CRITICAL: Force totals recalculation after setting shipping method
        $quote->setTotalsCollectedFlag(false);
        $quote->collectTotals();
        
        // Force another totals collection to ensure shipping is included
        $shippingAddress = $quote->getShippingAddress();
        if ($shippingAddress) {
            $shippingAddress->setCollectShippingRates(true);
            $quote->setTotalsCollectedFlag(false);
...
private function setShippingMethodOnQuote($quote, string $shippingMethod)
{
    ...
    if (!$quote->isVirtual() && $shippingAddress) {
        ...
        // CRITICAL: Ensure shipping amount is properly calculated and set
        $rates = $shippingAddress->getAllShippingRates();
        foreach ($rates as $rate) {
            if ($rate->getCode() === $shippingMethod) {
                $shippingCost = (float)$rate->getPrice();
                
                // Set the shipping amounts explicitly
                ...
                $this->logger->info('Shipping method set successfully', [
                    ...
                    'cost' => $shippingCost,
                    'set_amount' => $shippingAddress->getShippingAmount(),
                    'set_base_amount' => $shippingAddress->getBaseShippingAmount()
                ]);
                break;
            }
        }
...
/**
    ...
    private function extractCalculationResults($quote): array
    {
        ...
        $shippingAmount = 0.0;
        $discountAmount = 0.0;
        // Get shipping cost ONLY from quote - no fallbacks to avoid the 50 issue
        if (!$quote->isVirtual()) {
            $shippingAddress = $quote->getShippingAddress();
            
            // CRITICAL FIX: Use only the shipping amount set on the quote
            // Do NOT fall back to any default values that could be 50
            $shippingAmount = (float)$shippingAddress->getShippingAmount();
            
            // Only use base shipping amount if it's the same as regular amount
            $baseShippingAmount = (float)$shippingAddress->getBaseShippingAmount();
            if ($shippingAmount == 0 && $baseShippingAmount > 0) {
                $shippingAmount = $baseShippingAmount;
            }
            
            // Log shipping amount extraction for debugging
            $this->logger->error('CRITICAL: Final shipping amount extraction (NO FALLBACKS)', [
                'shipping_amount' => $shippingAddress->getShippingAmount(),
                'base_shipping_amount' => $shippingAddress->getBaseShippingAmount(),
                'final_shipping_amount' => $shippingAmount,
                'shipping_method' => $shippingAddress->getShippingMethod(),
                'grand_total' => $quote->getGrandTotal(),
                'subtotal' => $quote->getSubtotal(),
                'shipping_incl_tax' => $shippingAddress->getShippingInclTax(),
                'available_rates_count' => count($shippingAddress->getAllShippingRates())
            ]);
        }
        ...
        // Log final shipping calculation for debugging
        $this->logger->info('Final shipping cost in extractCalculationResults', [
            'quote_id' => $quote->getId(),
            'shipping_method' => $quote->getShippingAddress() ? $quote->getShippingAddress()->getShippingMethod() : 'none',
            'shipping_amount_from_address' => $quote->getShippingAddress() ? $quote->getShippingAddress()->getShippingAmount() : 0,
            'base_shipping_amount_from_address' => $quote->getShippingAddress() ? $quote->getShippingAddress()->getBaseShippingAmount() : 0,
            'final_shipping_amount' => $shippingAmount,
            'subtotal' => $subtotal,
            'grand_total' => $quote->getGrandTotal(),
...
private function createEnhancedQuoteForCalculation(int $productId, string $countryId, ?string $region = null, ?string $postcode = null, int $qty = 1)
{
    ...
    private function getFallbackCalculation(
        ...
        $this->logger->error('CRITICAL: Using getFallbackCalculation', [
            'product_id' => $productId,
            'qty' => $qty,
            'shipping_method' => $shippingMethod,
            'country_id' => $countryId,
        ...
        $this->logger->error('CRITICAL: getFallbackCalculation shipping cost result', [
            'shipping_cost' => $shippingCost,
            'shipping_method' => $shippingMethod,
            'product_price' => $productPrice,
            'subtotal' => $subtotal
